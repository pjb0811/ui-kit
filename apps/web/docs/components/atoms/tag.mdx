...
> Selectors like the one above must be updated; nothing else about the rendered
> output changed (same element, same classes). The old value is now free for the
> separate antd-style `Badge` — a count/dot decorator, which is a different
> component from this chip. The names cross between ecosystems: what shadcn
> calls a Badge is what antd (and this library) calls a Tag.
