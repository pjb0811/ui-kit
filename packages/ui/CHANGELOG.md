...
- 6694e68: Sync the package docs with the current dependency and export surface
...
- 3f4301b: Let `Reveals` and `Skeleton` shrink inside narrow containers
...
- bb7cb6d: Bump lucide-react to ^1.45.0
...
- 390cf0d: Make `core/checkbox` and `core/radio-group` upstream-verbatim (#361)
...
- 3fe67b5: Align `Switch.size` with the shared `ComponentSize` contract (#350)
...
- d4db4ca: Add a `RichTextEditor` atom, ported from `@jbpark/live-editor`'s
