...
각 컴포넌트는 `{계층}/{ComponentName}/index.tsx` 한 파일로 구성한다.
...
1. 컴포넌트 파일: `export default MyComponent` + `export type { Props }`
2. 계층 index: `src/components/{계층}/index.ts`에 `export { default as MyComponent } from './MyComponent'` 추가
3. 루트 `src/index.ts`는 계층 index를 re-export하므로 별도 수정 불필요

## apps/docs — Storybook 스토리 작성

파일 위치: `stories/{atoms|molecules|organisms}/{ComponentName}/index.stories.tsx`
